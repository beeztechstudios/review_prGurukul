...
const DEFAULT_MOODS = [
  { level: 1, emoji: "😡", label: "Very Unhappy", color: "from-red-400 to-red-600" },
  { level: 2, emoji: "😕", label: "Unhappy", color: "from-orange-400 to-orange-600" },
  { level: 3, emoji: "😐", label: "Neutral", color: "from-gray-400 to-gray-600" },
  { level: 4, emoji: "🙂", label: "Happy", color: "from-green-400 to-green-600" },
  { level: 5, emoji: "😍", label: "Very Happy", color: "from-blue-400 to-blue-600" },
];
  ...
  const MOODS =
    customMoods.length > 0
      ? customMoods.map((img: string, i: number) => ({
          level: i + 1,
          image: img,
          label: DEFAULT_MOODS[i]?.label || `Mood ${i + 1}`,
          color: DEFAULT_MOODS[i]?.color || "from-blue-400 to-blue-600",
        }))
      : DEFAULT_MOODS;
